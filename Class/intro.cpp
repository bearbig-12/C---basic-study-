// 1. 객체(Object)
//		현실세계에 존재하는 것을 프로그래밍상으로 가져온것. 상태와 행동을 가지고 있는건 (멤버 변수와 멤버함수)
// 
// 2. 클래스(Class)	- 객체를 프로그래밍으로 묶어놓은틀(상태와 행동 즉 멤버변수와 멤버 함수를 가지고 있는 틀)
//			상태(state)			행동(Behaviour)
// C++		멤버변수			멤버함수
// C#		필드(Field)			메써드(Method)
// Java		필드(Field)			메써드(Method)	
// 
// 2 - 1 Instance = 그 클래스를 바탕으로 찍어 낼 수 있는것
// 
// 3. 캡슐화(Capsulisation)
//		사용 편리 - 알 필요가 없음
//		보안	  -	내부 정보를 숨기는
